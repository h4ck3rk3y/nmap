...
--
-- @usage
-- nmap --script=http-wordpress-plugins --script-args http-wordpress-plugins.root="/blog/",http-wordpress-plugins.search=500 http://www.thisisyourkingdom.co.uk/ 
--
--@output
...
action = function(host, port)
  ...
  stdnse.print_debug(1,"plugins search range: %s", plugins_search or "unlimited")
  ...
  for i, data in pairs(pipeline_returns) do
    ...
    if http.page_exists(data, result_404, body_404, bfqueries[i][1], true) then
      stdnse.print_debug(1,"Found a plugin: %s", bfqueries[i][2])
